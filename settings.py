...
class Settings:
    ...
    def get_int_list(self, section, key):
        """
        Retrieves a list of integer values from the configuration. The values are expected to be
        stored as a string representation of a list (e.g., "[145, 100, 100]").
        ...
        string_value = self.config.get(section, key)
        values_as_strings = string_value.strip('[]').split(',')
        return [int(value.strip()) for value in values_as_strings]
