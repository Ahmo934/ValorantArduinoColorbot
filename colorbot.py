...
class Colorbot:
    ...
    def __init__(self, x, y, x_fov, y_fov):
        ...
        self.lower_color = np.array(self.settings.get_int_list('ColorRange', 'lower_color'))
        self.upper_color = np.array(self.settings.get_int_list('ColorRange', 'upper_color'))
        ...
        self.toggle_button = self.settings.get_int('ToggleButton', 'button')
    ...
    def listen_aimbot(self):
        ...
        """
        while True:
            if button_states[self.toggle_button]:
                self.process("move")
            time.sleep(0.01) 
